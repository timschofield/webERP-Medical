...
if (!isset($_POST['FromCat'])  OR $_POST['FromCat']=="") {
	$title="Inventory Planning Reporting";
}
...
strlen($_POST['ToCriteria'])>=1){
	...
	if ($_POST["Location"]=='All'){
		$SQL = "SELECT StockMaster.CategoryID, StockMaster.Description, StockCategory.CategoryDescription, LocStock.StockID, Sum(LocStock.Quantity) AS QOH FROM LocStock, StockMaster, StockCategory WHERE LocStock.StockID=StockMaster.StockID AND StockMaster.CategoryID=StockCategory.CategoryID AND (StockMaster.MBflag='B' OR MBflag='M') AND StockMaster.CategoryID >= '" . $_POST['FromCriteria'] . "' AND StockMaster.CategoryID <= '" . $_POST['ToCriteria'] . "' GROUP BY LocStock.StockID, StockCategory.CategoryDescription, StockMaster.Description, StockCategory.CategoryID ORDER BY StockMaster.CategoryID, StockMaster.StockID";
	} else {
		$SQL = $SQL = "SELECT StockMaster.CategoryID, LocStock.StockID, StockMaster.Description, StockCategory.CategoryDescription, LocStock.Quantity  AS QOH FROM LocStock, StockMaster, StockCategory WHERE LocStock.StockID=StockMaster.StockID AND StockMaster.CategoryID >= '" . $_POST['FromCriteria'] . "' AND StockMaster.CategoryID=StockCategory.CategoryID AND StockMaster.CategoryID <= '" . $_POST['ToCriteria'] . "' AND (StockMaster.MBflag='B' OR MBflag='M') AND LocStock.LocCode = '" . $_POST['Location'] . "' ORDER BY StockMaster.CategoryID, StockMaster.StockID";

	}
	$InventoryResult = DB_query($SQL,$db);

	if (DB_error_no($db) !=0) {
	  $title = "Inventory Planning - Problem Report.... ";
	  include("includes/header.inc");
	   echo "The inventory quantities could not be retrieved by the SQL because - " . DB_error_msg($db);
	...
	While ($InventoryPlan = DB_fetch_array($InventoryResult,$db)){
		...
		if (DB_error_no($db) !=0) {
	 		 $title = "Inventory Planning - Problem Report.... ";
	  		include("includes/header.inc");
	   		echo "<BR>The sales quantities could not be retrieved by the SQL because - " . DB_error_msg($db);
	   		echo "<BR><A HREF='" .$rootpath ."/index.php?" . SID . "'>Back to the menu</A>";
	   		if ($debug==1){
	      			echo "<BR>$SQL";
	   		}
		...
		if ($_POST["Location"]=='All'){
			$SQL = "SELECT Sum(SalesOrderDetails.Quantity - SalesOrderDetails.QtyInvoiced) AS QtyDemand FROM SalesOrderDetails, SalesOrders WHERE SalesOrderDetails.OrderNo=SalesOrders.OrderNo AND SalesOrderDetails.StkCode = '" . $InventoryPlan["StockID"] . "' AND SalesOrderDetails.Completed = 0";
		} else {
...
//Also need to add in the demand as a component of an assembly items if this items has any assembly parents.
		...
		if (DB_error_no($db) !=0) {
	 		$title = "Inventory Planning - Problem Report.... ";
	  		include("includes/header.inc");
	   		echo "<BR>The sales order demand quantities from parent assemblies could not be retrieved by the SQL because - " . DB_error_msg($db);
	   		echo "<BR><A HREF='" .$rootpath ."/index.php?" . SID . "'>Back to the menu</A>";
	   		if ($debug==1){
	      			echo "<BR>$SQL";
	   		}
		...
		if ($_POST["Location"]=='All'){
			...
			$SQL = "SELECT Sum(PurchOrderDetails.QuantityOrd - PurchOrderDetails.QuantityRecd) AS QtyOnOrder FROM PurchOrderDetails, PurchOrders WHERE PurchOrderDetails.OrderNo = PurchOrders.OrderNo AND PurchOrderDetails.ItemCode = '" . $InventoryPlan["StockID"] . "' AND PurchOrderDetails.Completed = 0 AND PurchOrders.IntoStockLocation=  '" . $_POST["Location"] . "'";
		}
		...
		$OnOrdResult = DB_query($SQL,$db);
		if (DB_error_no($db) !=0) {
	 		 $title = "Inventory Planning - Problem Report.... ";
	  		include("includes/header.inc");
	   		echo "<BR>The purchase order quantities could not be retrieved by the SQL because - " . DB_error_msg($db);
	   		echo "<BR><A HREF='" .$rootpath ."/index.php?" . SID . "'>Back to the menu</A>";
	   		if ($debug==1){
	      			echo "<BR>$SQL";
	   		}
	...
	if ($len<=20){
		$title = "Print Inventory Planning Report Empty";
		include("includes/header.inc");
		echo "<p>There were no items in the range and location specified";
		echo "<BR><A HREF='$rootpath/index.php?" . SID . "'>Back to the menu</A>";
		include("includes/footer.inc");
		exit;
	} else {
	...
	/*if $FromCriteria is not set then show a form to allow input	*/
		...
		echo "<TR><TD>Maximum No Months Holding:</TD><TD><SELECT name='NumberMonthsHolding'>";
		echo "<OPTION SELECTED Value=3>Three Months";
		echo "<OPTION Value=4>Four Months";
		echo "</SELECT></TD></TR>";
