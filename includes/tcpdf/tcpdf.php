	...
	*/
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		...
		protected function getHtmlDomArray($html) {
			...
			$offset = 0;
			while (($offset < strlen($html)) AND ($pos = mb_strpos($html, '</pre>', $offset)) !== false) {
			...
			$offset = 0;
			while (($offset < strlen($html)) AND ($pos = mb_strpos($html, '</textarea>', $offset)) !== false) {
				$html_a = mb_substr($html, 0, $offset);
			...
			$offset = 0;
			while (($offset < strlen($html)) AND ($pos = mb_strpos($html, '</option>', $offset)) !== false) {
				$html_a = mb_substr($html, 0, $offset);
							...
							if ($plalign == 'C') {
								...
								if (preg_match_all('/\[\(([^\)]*)\)\]/x', $pmidtemp, $lnstring, PREG_PATTERN_ORDER)) {
									...
									while (preg_match('/([0-9\.\+\-]*)[\s](Td|cm|m|l|c|re)[\s]/x', $pmid, $strpiece, PREG_OFFSET_CAPTURE, $offset) == 1) {
										...
										$epsposend = mb_strpos($pmid, $this->epsmarker.'Q', $offset) + strlen($this->epsmarker.'Q');
