...
/*Show a form to allow input of criteria for TB to show */
	...
	echo "<INPUT TYPE=SUBMIT Name='ShowTB' Value='Show Trial Balance'></CENTER>";
...
/*Now do the posting while the user is thinking about the period to select */
	...
	while ($myrow=DB_fetch_array($AccountsResult)) {
		...
		printf("<td>%s</FONT></td><td>%s</td><td ALIGN=RIGHT>%s</td><td ALIGN=RIGHT>%s</td><td ALIGN=RIGHT>%s</td><td ALIGN=RIGHT>%s</td></tr>",$ActEnquiryURL,$myrow["AccountName"],number_format($myrow["MonthActual"],2), number_format($myrow["MonthBudget"],2), number_format($AccountPeriodActual,2), number_format($AccountPeriodBudget,2));
	...
	//end of while loop
	printf("<tr bgcolor='#ffffff'><td COLSPAN=2><FONT COLOR=BLUE><B>Check Totals</B></FONT></td><td ALIGN=RIGHT>%s</td><td ALIGN=RIGHT>%s</td><td ALIGN=RIGHT>%s</td><td ALIGN=RIGHT>%s</td></tr>",number_format($CheckMonth,2),number_format($CheckBudgetMonth,2), number_format($CheckPeriodActual,2), number_format($CheckPeriodBudget,2));
	echo "</TABLE>";
