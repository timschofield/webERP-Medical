...
if ($_POST['submit']=="Enter Information") {
	...
	if (!is_numeric($_POST['QuantityBreak'])){
		echo "<BR>". _("The quantity break must be entered as a positive number");
		$InputError =1;
	}

	if ($_POST['QuantityBreak']<=0){
		echo "<BR>" . _("The quantity of all items on an order (in the discount category)") . "" . $_POST['DiscountCategory'] . "" . _(" at which the discount will apply is 0 or less than 0. Positive numbers are expected for this entry.");
		$InputError =1;
	}
	if (!is_numeric($_POST['DiscountRate'])){
		echo "<BR>" . _("The discount rate must be entered as a positive number");
		$InputError =1;
	}
	if ($_POST['DiscountRate']<=0 OR $_POST['DiscountRate']>=70){
		echo "<BR>" . _("The discount rate applicable for this record is either less than 0 % or greater than 70% numbers between 1 and 69 are expected");
		$InputError =1;
	}
...
/*the link to delete a selected record was clicked instead of the submit button */
	...
	$result = DB_query($sql,$db);
	echo "<BR>" . _("The discount matrix record has been deleted!") . " <p>";
}
...
echo "<CENTER><table>";
echo "<tr><td class='tableheader'>" . _("Sales Type") . "</td><td class='tableheader'>" . _("Discount Category") . "</td><td class='tableheader'>" . _("Quantity Break") . "</td><td class='tableheader'>" . _("Discount Rate %") . "</td>";
...
while ($myrow = DB_fetch_array($result)) {
	...
	printf("<td>%s</td><td>%s</td><td>%s</td><td>%s</td><td><a href='%s'>" . _("Delete") . "</td></tr>", $myrow['Sales_Type'], $myrow['DiscountCategory'],$myrow['QuantityBreak'],number_format($myrow['DiscountRate']*100,2) , $DeleteURL);
...
echo "<TR><TD>" . _("Customer Price List (Sales Type)") . ":</TD><TD>";
