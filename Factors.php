...
echo '<p class="page_title_text"><img src="'.$rootpath.'/css/'.$theme.'/images/supplier.png" title="' . _('Search') . '" alt="">' . ' ' . _('Factor Company Maintenance') . '';
...
if (!isset($FactorID) or ($FactorID == 1 and isset($_POST['amend']))) {
	...
	$NumberOfRows=DB_num_rows($result);
    ...
    echo "</select></td></tr></table><p><div class='centre'>";
	if ($NumberOfRows>0) {
		echo "<input tabindex=2 type='Submit' name='amend' VALUE='" . _('Amend Factor') . "'>";
	}
    echo "<input tabindex=3 type='Submit' name='Create' VALUE='" . _('Create New Factor') . "'>";
    echo '</div></form>';
} else {
    echo "<form method='post' action='" . $_SERVER['PHP_SELF'] . "?" . SID . "'>";
    echo '<table>';
