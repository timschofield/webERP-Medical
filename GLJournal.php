...
if (isset($_POST['JournalProcessDate'])){
	...
	if (!Is_Date($_POST['JournalProcessDate'])){
		echo "<BR><B><FONT SIZE=4 COLOR=RED>"._("WARNING: The date entered was not valid please enter the date to process the journal in the format")." $DefaultDateFormat";
		$_POST['CommitBatch']=_("Dont do it the date is wrong");
	}
}
...
*/
	...
	$result= DB_query($SQL,$db);
	if (DB_error_no($db)!=0){
		echo "<BR><B>"._("Problem Report").":</B><BR>"._("Cannot commit the changes").": -<BR>$SQL";
		$SQL = "Rollback";
		$result= DB_query($SQL,$db);
		exit;
	} else {

		echo "<P>"._("Journal")." " . $TransNo . " "._("has been sucessfully entered.");
  ...
  /* User hit delete the line from the journal */
   ...
   if ($_POST['GLManualCode']!="" AND is_numeric($_POST['GLManualCode'])){
	...
	if (in_array($_POST['GLManualCode'], $_SESSION['JournalDetail']->BankAccounts)) {
		echo '<P><FONT COLOR=RED SIZE=3>' . _('GL Journals involving a bank account cannot be entered. Bank account general ledger entries must be entered by either a bank account receipt or a bank account payment').'</FONT>';
	} else {
		$SQL = "SELECT AccountName FROM ChartMaster WHERE AccountCode=" . $_POST['GLManualCode'];
		$Result=DB_query($SQL,$db);
		if (DB_num_rows($Result)==0){
			echo "<BR>"._("The manual GL code entered does not exist in the database - so this GL analysis item could not be added.");
...
echo "<TR><TD>"._("Date to Process Journal").":</TD><TD><INPUT TYPE='text' name='JournalProcessDate' maxlength=10 size=11 value='" . $_SESSION['JournalDetail']->JnlDate . "'></TD></TR>";


echo '<TR><TD>' . _('Type:') . '</TD><TD><SELECT name=JournalType>';
...
echo '<TR><TD>' . _('Enter GL Account Manually') . ":</TD><TD><INPUT TYPE=Text Name='GLManualCode' Maxlength=12 SIZE=12 VALUE=" . $_POST['GLManualCode'] . "></TD>";
echo '<TD>'. _('OR Select GL Account').  ":</TD><TD><SELECT name='GLCode'>";
$SQL = "SELECT AccountCode, AccountName FROM ChartMaster ORDER BY AccountCode";
$result=DB_query($SQL,$db);
if (DB_num_rows($result)==0){
	echo "</SELECT>"._('No General ledger accounts have been set up yet - payments cannot be analysed against GL accounts until the GL accounts are set up.').'</TD></TR>';
} else {
...
echo "<TABLE WIDTH=100% BORDER=1><TR><td class='tableheader'>"._('Amount')."</TD><td class='tableheader'>"._('GL Account')."</TD><td class='tableheader'>"._('Narrative')."</TD></TR>";
...
if (ABS($_SESSION["JournalDetail"]->JournalTotal)<0.001 AND $_SESSION["JournalDetail"]->GLItemCounter > 0){
	...
	echo '<BR><BR>' . _('The journal must balance ie debits equal to credits before it can be processed').'</FONT>';
}
