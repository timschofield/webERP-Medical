...
if (isset($_POST['submit'])) {
	...
	if (!is_long((integer)$_POST['AccountCode'])) {
		$InputError = 1;
		echo _("The account code must be an integer.");
	} elseif (strlen($_POST['AccountName']) >50) {
		$InputError = 1;
		echo _("The account name must be fifty characters or less long");
	}
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'ChartDetails'
	...
	if ($myrow[0]>0) {
		$CancelDelete = 1;
		echo _("Cannot delete this account because chart details have been created using this account.");
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'GLTrans'
		...
		$ErrMsg = _("Couldn't test for existing transactions because");
		...
		if ($myrow[0]>0) {
			$CancelDelete = 1;
			echo _("Cannot delete this account because transactions have been created using this account.");
			...
			$ErrMsg = _("Couldn't test for default company GL codes because");
			$DbgMsg = _("The SQL that was used and failed was:");
			...
			if ($myrow[0]>0) {
				$CancelDelete = 1;
				echo _("Cannot delete this account because it is used as one of the company default accounts.");

			} else  {
				//PREVENT DELETES IF Company default accounts set up to this account
				$sql= "SELECT COUNT(*) FROM TaxAuthorities WHERE TaxGLCode=$SelectedAccount OR PurchTaxGLAccount =$SelectedAccount";

				$ErrMsg = _("Couldn't test for tax authority GL codes because");
				$DbgMsg = _("The SQL that was used and failed was:");
				$result = DB_query($sql,$db,$ErrMsg,$DbgMsg);
				...
				if ($myrow[0]>0) {
					$CancelDelete = 1;
					echo _("Cannot delete this account because it is used as one of the tax authority accounts.");
				} else {
//PREVENT DELETES IF SALES POSTINGS USE THE GL ACCOUNT
					...
					$ErrMsg = _("Couldn't test for existing sales interface GL codes because");
					$DbgMsg = _("The SQL that was used and failed was:");
					$result = DB_query($sql,$db,$ErrMsg,$DbgMsg);
					...
					if ($myrow[0]>0) {
						$CancelDelete = 1;
						echo "<BR>" . _("Cannot delete this account because it is used by one of the sales GL posting interface records.");
					} else {
//PREVENT DELETES IF COGS POSTINGS USE THE GL ACCOUNT
						$sql= "SELECT COUNT(*) FROM COGSGLPostings WHERE GLCode=$SelectedAccount";

						$ErrMsg = _("Couldn't test for existing cost of sales interface codes because");
						$Dbgmsg = _("The SQL that was used and failed was:");
						$result = DB_query($sql,$db,$ErrMsg,$DbgMsg);
						...
						if ($myrow[0]>0) {
							$CancelDelete = 1;
							echo "<BR>" . _("Cannot delete this account because it is used by one of the cost of sales GL posting interface records.");
...
//PREVENT DELETES IF STOCK POSTINGS USE THE GL ACCOUNT
							...
							$Errmsg = _("Couldn't test for existing stock GL codes because");
							$DbgMsg = _(">The SQL that was used and failed was:");
							...
							if ($myrow[0]>0) {
								$CancelDelete = 1;
								echo "<BR>" . _("Cannot delete this account because it is used by one of the stock GL posting interface records.");
							} else {
//PREVENT DELETES IF STOCK POSTINGS USE THE GL ACCOUNT
								$sql= "SELECT COUNT(*) FROM BankAccounts WHERE AccountCode=$SelectedAccount";
								$ErrMsg = _("Couldn't test for existing bank account GL codes because");
								$Dbgmsg = _("The SQL that was used and failed was:");
								$result = DB_query($sql,$db,$ErrMsg,$DbgMsg);
								...
								if ($myrow[0]>0) {
									$CancelDelete = 1;
									echo "<BR>" . _("Cannot delete this account because it is used by one the defined bank accounts.");
								} else {

									$sql="DELETE FROM ChartMaster WHERE AccountCode=$SelectedAccount";
									$result = DB_query($sql,$db);
									echo "<BR><CENTER><FONT COLOR=RED><B>" . _("Account") . $SelectedAccount . _("has been deleted !") . "</B></FONT></CENTER><p>";
								}
							}
						}
					}
...
or deletion of the records*/
	...
	echo "<tr>
		...
		<td class='tableheader'>Account Group</td>
		<td class='tableheader'>P/L Or B/S</td>
	</tr>";
	...
	printf("<td>%s</td>
		...
		<td>%s</td>
		<td>%s</td>
		<td><a href=\"%sSelectedAccount=%s\">Edit</td>
		<td><a href=\"%sSelectedAccount=%s&delete=1\">Delete</td>
		</tr>",
		$myrow[0],
		$myrow[1],
		$myrow[2],
