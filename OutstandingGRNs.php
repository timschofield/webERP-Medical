...
strlen($_POST['ToCriteria'])>=1){
	...
	$FontSize=10;
	$pdf->addinfo('Title',"Outstanding GRNs Report");
	$pdf->addinfo('Subject',"Outstanding GRNs Valuation");
	...
	$SQL = "SELECT GRNNo, OrderNo, GRNs.SupplierID, Suppliers.SuppName, GRNs.ItemCode, GRNs.ItemDescription, QtyRecd, QuantityInv, StdCostUnit, ActPrice, UnitPrice FROM GRNs, PurchOrderDetails, Suppliers WHERE GRNs.SupplierID=Suppliers.SupplierID AND GRNs.PODetailItem = PurchOrderDetails.PODetailItem AND QtyRecd-QuantityInv <>0 AND GRNs.SupplierID >='" . $_POST['FromCriteria'] . "' AND GRNs.SupplierID <='" . $_POST['ToCriteria'] . "' ORDER BY SupplierID, GRNNo";

	$GRNsResult = DB_query($SQL,$db);

	if (DB_error_no($db) !=0) {
	  $title = "Outstanding GRN Valuation - Problem Report.... ";
	  include("includes/header.inc");
	   echo "The outstanding GRNs valuation details could not be retrieved by the SQL because - " . DB_error_msg($db);
	...
	While ($GRNs = DB_fetch_array($GRNsResult,$db)){
		...
		if ($Supplier!=$GRNs['SupplierID']){
			...
			if ($Supplier!=""){ /*Then it's NOT the first time round */
				...
				$YPos -= (2*$line_height);
				$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos,260-$Left_Margin,$FontSize,"Total for " . $Supplier . " - " . $SupplierName);
...
/*Print out the supplier totals */
	$YPos -=$line_height;
	$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos,260-$Left_Margin,$FontSize,"Total for " . $Supplier . " - " . $SupplierName, "left");
...
/*Print out the grand totals */
	...
	$len = strlen($pdfcode);
      ...
      if ($len<=20){
		$title = "Outstanding GRNs Valuation Error";
		include("includes/header.inc");
		echo "<p>There were no GRNs with any value to print out for the specified supplier range";
		echo "<BR><A HREF='$rootpath/index.php?" . SID . "'>Back to the menu</A>";
		echo "</body</html>";
		exit;
      } else {
	...
	echo "<TR><TD>From Supplier Code:</TD><TD><INPUT TYPE=TEXT NAME='FromCriteria' VALUE='0'></TD></TR>";
	echo "<TR><TD>To Supplier Code:</TD><TD><INPUT TYPE=TEXT NAME='ToCriteria' VALUE='zzzzzzz'></TD></TR>";
