...
if (!isset($_POST['PostInvoice'])){
	...
	if (count( $_SESSION['SuppTrans']->GRNs)>0){   /*if there are any GRNs selected for invoicing then */
		...
		foreach ($_SESSION['SuppTrans']->GRNs as $EnteredGRN){
			...
			echo '<tr>
					<td>' . $EnteredGRN->GRNNo . '</td><td>' . $EnteredGRN->ItemCode .'</td>
		...
		echo '<tr><td colspan="5" class="number"><font color="blue">' . _('Total Value of Goods Charged') . ':</font></td>
			<td class="number"><font color="blue"><u>' . currency_number_format($TotalGRNValue,$_SESSION['SuppTrans']->CurrCode) . '</u></font></td></tr>';
		echo '</table>';
	}
	...
	if (count( $_SESSION['SuppTrans']->Shipts) > 0){   /*if there are any Shipment charges on the invoice*/
		...
		foreach ($_SESSION['SuppTrans']->Shipts as $EnteredShiptRef){
			...
			echo '<tr><td>' . $EnteredShiptRef->ShiptRef . '</td><td class="number">' .
				currency_number_format($EnteredShiptRef->Amount,$_SESSION['SuppTrans']->CurrCode) . '</td></tr>';
		...
		echo '<tr><td colspan="2" class="number"><font size="4" color="blue">' . _('Total') . ':</font></td>
			<td class="number"><font size="4" color="blue"><u>' .  currency_number_format($TotalShiptValue,$_SESSION['SuppTrans']->CurrCode) . '</u></font></td></tr></table>';
	}
	...
	if (count( $_SESSION['SuppTrans']->Assets) > 0){   /*if there are any fixed assets on the invoice*/
		...
		echo '<tr>
				...
				<td class="number"><font size="4" color="blue"><u>' .  currency_number_format($TotalAssetValue,$_SESSION['SuppTrans']->CurrCode) . '</u></font></td>
			</tr></table>';
	...
	if ( $_SESSION['SuppTrans']->GLLink_Creditors == 1){
		...
		if (count($_SESSION['SuppTrans']->GLCodes) > 0){
			...
			echo '<tr>
					...
					<td class="number">' .  currency_number_format($TotalGLValue,$_SESSION['SuppTrans']->CurrCode) . '</td>
				</tr></table>';
		}
		...
		echo '<br /><table class="selection">
					<tr>
						...
						<td colspan="2" class="number">' . currency_number_format( $_SESSION['SuppTrans']->OvAmount,$_SESSION['SuppTrans']->CurrCode) . '</td>
					</tr>';
	} else {
		echo '<br /><table class="selection">
					<tr>
						<td>' . _('Amount in supplier currency') . ':</td>
						<td colspan="2" class="number"><input type="text" size="12" maxlength="10" name="OvAmount" value="' . currency_number_format( $_SESSION['SuppTrans']->OvAmount,$_SESSION['SuppTrans']->CurrCode) . '" /></td>
					</tr>';
	}
					...
					if ($EnteredGRN->StdCostUnit * ($EnteredGRN->This_QuantityInv ) != 0) {
						$SQL = "INSERT INTO gltrans (type,
												...
												VALUES (
													...
													'" . $PeriodNo . "',
													'" . $_SESSION['SuppTrans']->GRNAct . "',
													'" . $_SESSION['SuppTrans']->SupplierID . ' - ' . _('GRN') . ' ' . $EnteredGRN->GRNNo . ' - ' . $EnteredGRN->ItemCode .
														' x ' . $EnteredGRN->This_QuantityInv . ' @  ' .  _('std cost of') . ' ' . $EnteredGRN->StdCostUnit  . "',
                                	...
                                	$SQL = "INSERT INTO gltrans (type,
															...
															VALUES (
																...
																'" . $_SESSION['SuppTrans']->SupplierID . ' - ' . _('Inv') . ' ' .
																	 $_SESSION['SuppTrans']->SuppReference . ' ' . $Tax->TaxAuthDescription . ' ' . currency_number_format($Tax->TaxRate*100,$_SESSION['SuppTrans']->CurrCode) . '% ' . $_SESSION['SuppTrans']->CurrCode .
			...
			$SQL = "INSERT INTO gltrans (type,
									...
									VALUES (
										...
										'" . $_SESSION['SuppTrans']->SupplierID . ' - ' . _('Inv') . ' ' .
											 ...
											 currency_number_format( $_SESSION['SuppTrans']->OvAmount + $TaxTotal,$_SESSION['SuppTrans']->CurrCode)  .
