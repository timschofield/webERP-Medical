...
if (isset($_POST['submit'])) {
	...
	if ($myrowPcTabExpenses[0]>0 or $myrowPcTabs[0]>0) {
		...
		echo '<br /><div class="centre"><button type="submit" name=return value="' . _('Return to list of tab types') . '" /></button></div>';
		echo '</form>';
...
or deletion of the records*/
	...
	while ($myrow = DB_fetch_array($result)) {
		...
		printf('<td>%s</td>
				...
				htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). '?', $myrow['typetabcode'],
				htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). '?', $myrow['typetabcode']);
	}
	//END WHILE LIST LOOP
	echo '</table>';
