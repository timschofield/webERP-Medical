...
echo '<p class="page_title_text">
		<img src="', $RootPath, '/css/', $_SESSION['Theme'], '/images/maintenance.png" title="', _('Outcome Types'), '" alt="" />', _('Measurement Type Setup'), '
	</p>';
...
if (isset($_POST['Insert']) or isset($_POST['Update'])) {
	...
	if (isset($_POST['Update']) and $InputError != 1) {
		...
		$SQL = "UPDATE care_type_unit_measurement
				SET type = '" . $_POST['Type'] . "',
					name = '" . $_POST['TypeName'] . "',
		...
		$CheckSql = "SELECT count(type)
						FROM care_type_unit_measurement
		...
		if ($CheckRow[0] > 0 and !isset($SelectedType)) {
			...
			$SQL = "INSERT INTO care_type_unit_measurement (nr,
													type,
													name,
													description,
													status,
	...
	$SQL = "DELETE FROM care_type_unit_measurement WHERE nr='" . $SelectedType . "'";
