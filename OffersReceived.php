...
if (!isset($_POST['submit']) and isset($_POST['supplierid'])) {
	...
	while ($myrow=DB_fetch_array($result)) {
		...
		echo '<td class="number">'.locale_number_format($myrow['quantity'],$myrow['decimalplaces']).'</td>';
		echo '<td>'.$myrow['uom'].'</td>';
		echo '<td class="number">'.locale_number_format($myrow['price'],4).'</td>';
		echo '<td class="number">'.locale_money_format($myrow['price']*$myrow['quantity'],$myrow['currcode']).'</td>';
