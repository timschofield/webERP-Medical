...
If (isset($_POST['PrintPDF']) AND Is_Date($_POST['FromDate']) AND Is_Date($_POST['ToDate'])){
	...
	$FontSize=10;
	$pdf->addinfo('Title',"Low Gross Profit Sales");
	$pdf->addinfo('Subject',"Low Gross Profit Sales");

	$PageNumber=1;
	$line_height=12;

      /*Now figure out the data to report for the category range under review */
	$SQL = "SELECT StockMaster.CategoryID, StockMaster.StockID, StockMoves.TransNo, StockMoves.TranDate, SysTypes.TypeName, StockMaster.MaterialCost + StockMaster.LabourCost + StockMaster.OverheadCost AS UnitCost, StockMoves.Qty, StockMoves.DebtorNo, StockMoves.BranchCode, StockMoves.Price*(1-StockMoves.DiscountPercent) AS SellingPrice, (StockMoves.Price*(1-StockMoves.DiscountPercent)) - (StockMaster.MaterialCost + StockMaster.LabourCost + StockMaster.OverheadCost) AS GP FROM StockMaster, StockMoves, SysTypes WHERE StockMoves.Type=SysTypes.TypeID AND StockMaster.StockID=StockMoves.StockID AND StockMoves.TranDate >= '" . FormatDateForSQL($_POST['FromDate']) . "' AND StockMoves.TranDate <= '" . FormatDateForSQL($_POST['ToDate']) . "' AND ((StockMoves.Price*(1-StockMoves.DiscountPercent)) - (StockMaster.MaterialCost + StockMaster.LabourCost + StockMaster.OverheadCost))/(StockMoves.Price*(1-StockMoves.DiscountPercent)) <=" . ($_POST['GPMin']/100) . " ORDER BY StockMaster.StockID";

	$LowGPSalesResult = DB_query($SQL,$db);

	if (DB_error_no($db) !=0) {
	  $title = "Low GP sales - Problem Report.... ";
	  include("includes/header.inc");
	   echo "<BR>The low GP items could not be retrieved by the SQL because - " . DB_error_msg($db);
	...
	$len = strlen($pdfcode);
      ...
      if ($len<=20){
		$title = "Print Low GP Items Error";
		include("includes/header.inc");
		echo "<p>There were no items below  print out for the location specified";
		echo "<BR><A HREF='$rootpath/index.php?" . SID . "'>Back to the menu</A>";
		include("includes/footer.inc");
		exit;
      } else {
	...
	/*if $FromDate is not set then show a form to allow input */
		...
		echo "<TR><TD>Sales Made From (in the format $DefaultDateFormat):</TD><TD><INPUT TYPE=TEXT NAME='FromDate' SIZE=10 MAXLENGTH=10 VALUE='" . $_POST['FromDate'] . "'></TD></TR>";

		echo "<TR><TD>Sales Made To (in the format $DefaultDateFormat):</TD><TD><INPUT TYPE=TEXT NAME='ToDate' SIZE=10 MAXLENGTH=10 VALUE='" . $_POST['ToDate'] . "'></TD></TR>";

		echo "<TR><TD>Show sales with GP % below:</TD><TD><INPUT TYPE=text NAME='GPMin' MAXLENGTH=3 SIZE=3 VALUE=" . $_POST['GPMin'] . "></TD></TR>";
