...
$Title = _('Feeding Types') . ' / ' . _('Maintenance');
...
echo '<p class="page_title_text">
		<img src="', $RootPath, '/css/', $_SESSION['Theme'], '/images/maintenance.png" title="', _('Feeding Types'), '" alt="" />', _('Feeding Type Setup'), '
	</p>';
echo '<div class="page_help_text">', _('Add/edit/delete Feeding Types'), '</div>';
...
if (isset($_POST['Insert']) or isset($_POST['Update'])) {
	...
	if (mb_strlen($_POST['TypeName']) > 100) {
		$InputError = 1;
		prnMsg(_('The feeding type name description must be 100 characters or less long'), 'error');
	}

	if (mb_strlen($_POST['TypeName']) == 0) {
		$InputError = 1;
		prnMsg(_('The feeding type name description must contain at least one character'), 'error');
	}

	if (isset($_POST['Update']) and $InputError != 1) {

		$SQL = "UPDATE care_type_feeding
				SET type = '" . $_POST['Type'] . "',
					name = '" . $_POST['TypeName'] . "',
		...
		$Msg = _('The feeding type') . ' ' . $SelectedType . ' ' . _('has been updated');
	} elseif ($InputError != 1) {

		$CheckSql = "SELECT count(type)
						FROM care_type_feeding
		...
		if ($CheckRow[0] > 0 and !isset($SelectedType)) {
			$InputError = 1;
			echo '<br />';
			prnMsg(_('You already have a feeding type called') . ' ' . $_POST['TypeName'], 'error');
		} else {

			// Add new record on submit
			$SQL = "INSERT INTO care_type_feeding (nr,
													type,
													name,
													description,
													status,
			...
			$Msg = _('Feeding type') . ' ' . $_POST['TypeName'] . ' ' . _('has been created');
		}
	...
	$SQL = "DELETE FROM care_type_feeding WHERE nr='" . $SelectedType . "'";
	$ErrMsg = _('The Type record could not be deleted because');
	$Result = DB_query($SQL, $ErrMsg);
	echo '<br />';
	prnMsg(_('The feeding type has been deleted'), 'success');
	...
	if (DB_num_rows($Result) > 0) {
		...
		while ($MyRow = DB_fetch_array($Result)) {
			echo '<tr class="striped_row">
					...
					<td><a href="', htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'), '?SelectedType=', urlencode($MyRow['nr']), '&amp;delete=yes" onclick="return MakeConfirm(\'' . _('Are you sure you wish to delete this Feeding Type?') . '\', \'Confirm Delete\', this);">' . _('Delete') . '</a></td>
				</tr>';
		}
		//END WHILE LIST LOOP
		echo '</tbody>';
...
if (!isset($_GET['delete'])) {
	...
	if (isset($SelectedType) and $SelectedType != '') {
		...
		echo '<fieldset>
				<legend>', _('Edit Feeding Type'), ' - ', $MyRow['type'], '</legend>';
		...
		echo '<fieldset>
				<legend>', _('Create New Feeding Type'), '</legend>';
	}
	...
	if (isset($SelectedType) and $SelectedType != '') {
		echo '<div class="centre">
				<input type="submit" name="Update" value="', _('Update Feeding Type'), '" />
			</div>';
	} else {
		echo '<div class="centre">
				<input type="submit" name="Insert" value="', _('Create Feeding Type'), '" />
			</div>';
	}
