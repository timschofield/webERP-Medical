...
if ($FromTransNo=='Preview') {
	$FormDesign = simplexml_load_file(sys_get_temp_dir().'/InsuranceInvoice.xml');
} else {
	$FormDesign = simplexml_load_file($PathPrefix.'companies/'.$_SESSION['DatabaseName'].'/FormDesigns/InsuranceInvoice.xml');
}
	...
	while ($FromTransNo <= $_POST['ToTransNo']) {
		...
		if ($FromTransNo=='Preview' or DB_num_rows($result) == 1) {
			...
			if ($FromTransNo=='Preview' or DB_num_rows($result) > 0) {
				$PageNumber = 1;
				include ('includes/PDFInsuranceInvoiceHeader.inc');
				$FirstPage = False;
					...
					for ($i = 0;$i < sizeOf($lines);$i++) {
						while (strlen($lines[$i]) > 1) {
							if ($YPos - $line_height <= $Bottom_Margin) {
								...
								include ('includes/PDFInsuranceInvoice.inc');
