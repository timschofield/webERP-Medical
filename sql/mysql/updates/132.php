...
CONSTRAINT `bomheader_ibfk_3` FOREIGN KEY (`loccode`) REFERENCES `locations` (`loccode`)
...
$sql="INSERT INTO `bomheader` (`productid`, `uom`, `revisionlevel`, `loccode`) (SELECT DISTINCT parent, (SELECT units FROM stockmaster WHERE stockid=bom.parent), 1, loccode FROM `bom`)";

$result=DB_query($sql, $db);

$sql="INSERT INTO `bomlines` (`headerid`, `componentid`, `uom`, `revisionlevel`, `quantity`, `effectivefrom`, `effectiveto`, `workcentreadded`, `autoissue`)
	(SELECT (SELECT bomid FROM bomheader WHERE productid=bom.parent), component, (SELECT units FROM stockmaster WHERE stockid=bom.parent), 1, quantity, effectiveafter, effectiveto, workcentreadded, autoissue FROM `bom`)";
...
AS SELECT bomheader.productid, bomlines.componentid, bomlines.workcentreadded, bomheader.loccode, bomlines.effectivefrom, bomlines.effectiveto,
