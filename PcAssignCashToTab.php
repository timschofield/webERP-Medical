...
$title = _('Assignation of Cash to Petty Cash Tab');
...
if (isset($_POST['submit'])) {
	...
	if ($InputError !=1 AND isset($SelectedIndex) ) {
		...
		$msg = _('Assignation of cash to PC Tab ') . ' ' . $_POST["SelectedTabs"] .  ' ' . _('has been created');
	}
	...
	$sql="DELETE FROM pcashdetails
		WHERE counterindex='$SelectedIndex'";
	$ErrMsg = _('The assignation of cash record could not be deleted because');
	$result = DB_query($sql,$db,$ErrMsg);
	prnMsg(_('Assignation of cash to PC Tab ') .  ' ' . $SelectedTabs  . ' ' . _('has been deleted') ,'success');
	unset($_GET['delete']);
}
