...
if (isset($SelectedPatient) and isset($_GET['Edit'])) {
	$SQL = "SELECT current_ward_nr FROM care_encounter WHERE pid='" . $SelectedPatient . "' AND is_discharged=0 AND current_ward_nr<>0";
	$Result = DB_query($SQL);
	if (DB_num_rows($Result) == 0) {
		prnMsg(_('This patient is not currently admitted as an inpatient'), 'error');
...
if (isset($_GET['Edit'])) {
	echo '<fieldset>
			<legend>', _('Update Admission data for'), ' ', $MyRow['name'], ' (PID - ', $SelectedPatient, ') ', '</legend>';
} else {
	echo '<fieldset>
			<legend>', _('Admit'), ' ', $MyRow['name'], ' (PID - ', $SelectedPatient, ') ', _('as an inpatient'), '</legend>';
}
...
while ($MyRow = DB_fetch_array($Result)) {
	if ($EncounterRow['admit_type'] == $MyRow['type_nr']) {
