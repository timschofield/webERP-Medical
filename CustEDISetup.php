...
if (isset($_POST['submit'])) {
    ...
    if (strstr($_POST['EDIReference'],"'")
		...
		OR strstr($_POST['EDIReference'],' ')) {
        $InputError = 1;
        prnMsg(_('The customers EDI reference code cannot contain any of the following characters - \' & + \" or a space'),'warn');
    }
...
echo "</SELECT><A HREF='$rootpath/EDIMessageFormat.php?" . SID . "&MessageType=INVOIC&PartnerCode=" . $_SESSION['CustomerID'] . "'>"._('Create/Edit Invoice Message Format').'</A></TD></TR>';
...
echo '<TR><TD>'._('Customer\'s EDI Reference:')."</TD>
...
echo '<TR><TD>'._('FTP Server/Email Address:')."</TD>
