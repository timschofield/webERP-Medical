 ...
 */
	function InsertSalesCredit($CreditDetails, $user, $password) {
		...
		$FieldNames='';
		$FieldValues='';
		$CreditDetails['trandate']=ConvertSQLDate($CreditDetails['trandate']);
		foreach ($CreditDetails as $key => $value) {
			$FieldNames.=$key.', ';
			$FieldValues.='"'.$value.'", ';
		}
		if (sizeof($Errors)==0) {
			...
			$sql='insert into gltrans Values("null", 11,'.GetNextTransactionNo(11, $db).
				...
				'", '.intval($CreditDetails['ovamount']).', 1,"'.$CreditDetails['jobref'].'")';
			$result = DB_Query($sql, $db);
			$sql = 'UPDATE systypes set typeno='.GetNextTransactionNo(11, $db).' where typeid=11';
			$result = DB_Query($sql, $db);
			if (DB_error_no($db) != 0) {
				$Errors[0] = DatabaseUpdateFailed;
			} else {
