...
if (!isset($StockID)) {
	...
	echo '<tr><th colspan="6" class="header">' . _('To search for sales orders for a specific part use the part selection facilities below');
	echo '</th></tr>';
	echo '<tr>
...
if (isset($StockItemsResult) and DB_num_rows($StockItemsResult)>0) {
	...
	while ($myrow=DB_fetch_array($StockItemsResult)) {
		...
		printf('<td><button type="submit" name="SelectedStockItem" value="%s">%s</td>
				<td>%s</td>
	...
	if (DB_num_rows($SalesOrdersResult)>0) {
		...
		if ($_POST['Quotations']=='Orders_Only'  AND $AuthRow['cancreate']==0){ //cancreate==0 means can create POs
			echo '<tr><th colspan="8"></th><th></th><th colspan="2" class="number">
				<button type="submit" name="PlacePO" value="" onclick="return confirm(\'' . _('This will create purchase orders for all the items on the checked sales orders above, based on the preferred supplier purchasing data held in the system. Are You Absolutely Sure?') . '\');">' . _('Place') . "\n" . _('PO') . '</button></th></tr>';
		}
