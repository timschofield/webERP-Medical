...
$Title = _('Maintain Bacteriology Laboratory Tests');
$ViewTopic = '';
$BookMark = '';
...
if (isset($_POST['Update'])) {
	$UpdateSQL = "UPDATE care_baclabor_test_type SET name='" . $_POST['Name'] . "',
...
if (isset($_POST['Insert'])) {
	$InsertSQL = "INSERT INTO care_baclabor_test_type (type,
														name,
														stockid,
														create_time,
														create_id,
	...
	if (DB_error_no() == 0) {
		prnMsg(_('Bacteriology test details have been created'), 'success');
	} else {
		prnMsg(_('Bacteriology test details could not be created'), 'error');
	}

}

if (isset($_GET['Delete'])) {
	$DeleteSQL = "DELETE FROM care_baclabor_test_type WHERE type='" . $_GET['type'] . "'";
	$DeleteResult = DB_query($DeleteSQL);
	if (DB_error_no() == 0) {
		prnMsg(_('Bacteriology test has been deleted'), 'success');
	} else {
		prnMsg(_('Bacteriology test could not be creatdeleteded'), 'error');
	}
}
...
if (isset($_GET['Edit'])) {
	...
	echo '<fieldset>
			<legend>', _('Edit Bacteriology Test Details'), '</legend>';
	...
	$SQL = "SELECT stockid, description FROM stockmaster WHERE categoryid='" . $_SESSION['bacteriology_cat'] . "' ORDER BY description";
	...
	echo '<fieldset>
			<legend>', _('Create Bacteriology Test Details'), '</legend>';
