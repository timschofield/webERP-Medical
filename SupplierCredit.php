...
/*Now retrieve supplier information - name, currency, default ex rate, terms, tax rate etc */
	...
	echo '<P>' . _('To enter a supplier credit note the supplier must first be selected from the supplier selection screen.');
...
if ($_POST['GRNS'] == _('Enter Credit Against Goods Recd')){
	...
	echo '<P>' . _("You should automatically be forwarded to the entry of credit notes against goods received page. 
						If this does not happen (if the browser doesn't support META Refresh)") . 
						' ' . "<A HREF='" . $rootpath . "/SuppCreditGRNs.php?" . SID . "'>" . 
						_('click here') . '</A> ' . _('to continue') . '.<BR>';
	exit;
}
if ($_POST['Shipts'] == _('Enter Credit Against Shipment')){
...
echo "<CENTER><TABLE BORDER=2 COLSPAN=4><TR><TD CLASS='tableheader'>" . _('Supplier') . 
	  "</TD><TD CLASS='tableheader'>" . _('Currency') . "</TD><TD CLASS='tableheader'>" . 
	  _('Terms') . "</TD><TD CLASS='tableheader'>" . _('Tax Authority') . '</TD></TR>';

echo '<TR><TD><FONT COLOR=red><B>' . $_SESSION['SuppTrans']->SupplierID . ' - ' . 
		$_SESSION['SuppTrans']->SupplierName . '</TD><TD ALIGN=CENTER><FONT COLOR=red><B>' . 
		$_SESSION['SuppTrans']->CurrCode . '</TD><TD><FONT COLOR=red><B>' . 
		$_SESSION['SuppTrans']->TermsDescription . '</TD><TD ALIGN=CENTER><FONT COLOR=red><B>' . 
		$_SESSION['SuppTrans']->TaxDescription . ' (' . 
		(($_SESSION['SuppTrans']->TaxRate)*100) . '%)</TD></TR></TABLE></B></FONT>';

echo "<FORM ACTION='" . $_SERVER['PHP_SELF'] . "?" . SID . "' METHOD=POST>";

echo '<TABLE>';
echo '<TR><TD><FONT COLOR=red>' . _("Supplier's Credit Note Reference") . ":</FONT></TD>
		<TD><FONT SIZE=2><INPUT TYPE=TEXT SIZE=20 MAXLENGTH=20 NAME=SuppReference VALUE='" . 
		$_SESSION['SuppTrans']->SuppReference . "'></TD></TR>";
...
echo '<TR><TD><FONT COLOR=red>' . _('Credit Note Date') . ' (' . _('in format') . ' ' . 
		$DefaultDateFormat . ") :</FONT></TD><TD><INPUT TYPE=TEXT SIZE=11 MAXLENGTH=10 NAME='TranDate' VALUE=" . 
		$_SESSION['SuppTrans']->TranDate . '></TD></TR>';
echo '<TR><TD><FONT COLOR=red>' . _('Exchange Rate') . ":</FONT></TD>
		<TD><INPUT TYPE=TEXT SIZE=11 MAXLENGTH=10 NAME='ExRate' VALUE=" . 
		$_SESSION['SuppTrans']->ExRate . '></TD></TR>';
echo '</TABLE>';
...
if (count($_SESSION['SuppTrans']->GRNs)>0){   /*if there are some GRNs selected for crediting then */
	...
	$TableHeader = "<TR><TD CLASS='tableheader'>" . _('GRN') . "</TD><TD CLASS='tableheader'>" . _('Item Code') . "</TD><TD CLASS='tableheader'>" . _('Description') . "</TD><TD CLASS='tableheader'>" . _('Quantity') . '<BR>' . _('Credited') . "</TD><TD CLASS='tableheader'>" . _('Price Credited') . '<BR>' . _('in') . ' ' . $_SESSION['SuppTrans']->CurrCode . "</TD><TD CLASS='tableheader'>" . _('Line Total') . '<BR>' . _('in') . ' ' . $_SESSION['SuppTrans']->CurrCode . '</TD></TR>';
	echo $TableHeader;
	$TotalGRNValue=0;

	foreach ($_SESSION['SuppTrans']->GRNs as $EnteredGRN){

		echo '<TR><TD>' . $EnteredGRN->GRNNo . '</TD><TD>' . $EnteredGRN->ItemCode . '</TD><TD>' . 
				$EnteredGRN->ItemDescription . '</TD><TD ALIGN=RIGHT>' . number_format($EnteredGRN->This_QuantityInv,2) . 
				'</TD><TD ALIGN=RIGHT>' . number_format($EnteredGRN->ChgPrice,2) . '</TD><TD ALIGN=RIGHT>' . 
	...
	echo '<TR><TD COLSPAN=5 ALIGN=RIGHT><FONT COLOR=red>' . _('Total Value of Goods Credited') . 
		  ':</FONT></TD><TD ALIGN=RIGHT><FONT COLOR=red><U>' . number_format($TotalGRNValue,2) . '</U></FONT></TD></TR>';
	echo '</TABLE>';
}

if (count($_SESSION['SuppTrans']->Shipts)>0){   /*if there are any Shipment charges on the credit note*/

	echo "<TABLE CELLPADDING=2><TR><TD CLASS='tableheader'>" . _('Shipment') . "</TD><TD CLASS='tableheader'>" . 
	...
	echo '<TR><TD COLSPAN=2 ALIGN=RIGHT><FONT SIZE=4 COLOR=red>' . _('Total Credited Against Shipments') . 
		  ':</FONT></TD><TD ALIGN=RIGHT><FONT SIZE=4 COLOR=red><U>' . number_format($TotalShiptValue,2) . 
		  '</U></FONT></TD></TR>';
}
...
if ($_SESSION['SuppTrans']->GLLink_Creditors ==1){
	...
	if (count($_SESSION['SuppTrans']->GLCodes)>0){
		echo '<TABLE CELLPADDING=2>';
		$TableHeader = "<TR><TD CLASS='tableheader'>" . _('Account') . "</TD><TD CLASS='tableheader'>" . 
							_('Name') . "</TD><TD CLASS='tableheader'>" . _('Amount') . '<BR>' . _('in') . ' ' . 
							$_SESSION['SuppTrans']->CurrCode . "</TD><TD CLASS='tableheader'>" . _('Shipment') . 
							"</TD><TD CLASS='tableheader'>" . _('Job') . "</TD><TD CLASS='tableheader'>" . _('Narrative') . 
							'</TD></TR>';
		echo $TableHeader;
		...
		foreach ($_SESSION['SuppTrans']->GLCodes as $EnteredGLCode){
			...
			echo '<TR><TD>' . $EnteredGLCode->GLCode . '</TD><TD>' . $EnteredGLCode->GLActName . '</TD><TD ALIGN=RIGHT>' . 
					number_format($EnteredGLCode->Amount,2) . '</TD><TD>' . $EnteredGLCode->ShiptRef . '</TD><TD>' . 
					$EnteredGLCode->JobRef . '</TD><TD>' . $EnteredGLCode->Narrative . '</TD></TR>';
		...
		echo '<TR><TD COLSPAN=2 ALIGN=RIGHT><FONT SIZE=4 COLOR=red>' . _('Total') . 
			  ':</FONT></TD><TD ALIGN=RIGHT><FONT SIZE=4 COLOR=red><U>' . 
	...
	echo '<TABLE><TR><TD><FONT COLOR=red>' . _("Credit Amount in Supplier's Currency") . 
		  ':</FONT></TD><TD ALIGN=RIGHT>' . number_format($_SESSION['SuppTrans']->OvAmount,2) . '</TD></TR>';
} else {
	echo '<TABLE><TR><TD><FONT COLOR=red>' . _("Credit Amount in Supplier's Currency") . 
		  ':</FONT></TD><TD ALIGN=RIGHT><INPUT TYPE=TEXT SIZE=12 MAXLENGTH=10 NAME=OvAmount VALUE=' . 
...
then do the updates and inserts to process the credit note entered */
	...
	If ($_SESSION['SuppTrans']->OvGST + $_SESSION['SuppTrans']->OvAmount <= 0){
		$InputError = True;
		echo '<P>' . _('The credit note as entered cannot be processed because the total amount of the credit note is less than or equal to 0! 
							Credit notes are expected to entered as positive amounts to credit.');
	} elseif (strlen($_SESSION['SuppTrans']->SuppReference) < 1){
		$InputError = True;
		echo '<P>' . _('The credit note as entered cannot be processed because the there is no suppliers credit note number or reference entered. 
							The supplier credit note number must be entered.');
	} elseif (!is_date($_SESSION['SuppTrans']->TranDate)){
		$InputError = True;
		echo '<P>' . _('The credit note as entered cannot be processed because the date entered is not in the format') . 
				' ' . $DefaultDateFormat;
	} elseif (DateDiff(Date($DefaultDateFormat), $_SESSION['SuppTrans']->TranDate, "d") < 0){
		$InputError = True;
		echo '<P>' . _('The credit note as entered cannot be processed because the date is after today. 
							Purchase credit notes are expected to have a date prior to or today');
	}elseif ($_SESSION['SuppTrans']->ExRate <= 0){
		$InputError = True;
		echo '<P>' . _('The credit note as entered cannot be processed because the exchange rate for the credit note has been entered as a negative or zero number. 
				The exchange rate is expected to show how many of the suppliers currency there are in 1 of the local currency.');
	}elseif ($_SESSION['SuppTrans']->OvAmount < round($TotalShiptValue + $TotalGLValue + $TotalGRNValue,2)){
		echo '<P>' . _('The credit note total as entered is less than the sum of the shipment charges, the general ledger entires (if any) and the charges for goods received. 
				There must be a mistake somewhere, the credit note as entered will not be processed.');
		$InputError = True;
	} else {
		...
		/*Loop through the GL Entries and create a debit posting for each of the accounts entered */
			...
			*/
			...
			the debit is to creditors control act  done later for the total credit note value + tax*/
				...
				$SQL = 'INSERT INTO GLTrans (Type, TypeNo, TranDate, PeriodNo, Account, Narrative, Amount, JobRef) 
						 VALUES (21, ' . $CreditNoteNo . ", '" . $SQLCreditNoteDate . "', " . $PeriodNo . ', ' . 
						 $EnteredGLCode->GLCode . ", '" . $_SESSION['SuppTrans']->SupplierID . " " . 
						 $EnteredGLCode->Narrative . "', " . round(-$EnteredGLCode->Amount/$_SESSION['SuppTrans']->ExRate,2) . 
			...
			these entries are reversed from the GRN suspense when the shipment is closed - entries only to open shipts*/
				...
				$SQL = 'INSERT INTO GLTrans (Type, TypeNo, TranDate, PeriodNo, Account, Narrative, Amount) VALUES (21, ' . 
						 $CreditNoteNo . ", '" . $SQLCreditNoteDate . "', " . $PeriodNo . ', ' . 
						 $_SESSION['SuppTrans']->GRNAct . ", '" . $_SESSION['SuppTrans']->SupplierID . ' ' . 
						 _('Shipment credit against') . ' ' . $ShiptChg->ShiptRef . "', " . 
			...
			foreach ($_SESSION['SuppTrans']->GRNs as $EnteredGRN){
				...
				if (strlen($EnteredGRN->ShiptRef)==0 OR $EnteredGRN->ShiptRef=="" OR $EnteredGRN->ShiptRef==0){ /*so its not a shipment item */
					...
					if ($PurchPriceVar !=0){ /* don't bother with this lot if there is no value to post ! */
						if (strlen($EnteredGRN->ItemCode)>0 OR $EnteredGRN->ItemCode!=""){ /*so it is a stock item */
							...
							$SQL = 'INSERT INTO GLTrans (Type, TypeNo, TranDate, PeriodNo, Account, Narrative, Amount) 
									 VALUES (21, ' . $CreditNoteNo . ", '" . $SQLCreditNoteDate . "', " . $PeriodNo . ', ' . 
									 $StockGLCode['PurchPriceVarAct'] . ", '" . $_SESSION['SuppTrans']->SupplierID . ' - ' . 
									 _('GRN Credit Note') . ' ' . $EnteredGRN->GRNNo . ' - ' . $EnteredGRN->ItemCode . ' x ' . 
									 $EnteredGRN->This_QuantityInv . ' x  ' . 
									 number_format(($EnteredGRN->ChgPrice  / $_SESSION['SuppTrans']->ExRate),2)  . "', " . 
									 (-$PurchPriceVar) . ')';

							$ErrMsg = _('ERROR! NOTE DOWN THIS ERROR AND SEEK ASSISTANCE: 
											The general ledger transaction could not be added for the price variance of the stock item, because');
						...
						the goods were received */
							...
							$SQL = 'INSERT INTO GLTrans (Type, TypeNo, TranDate, PeriodNo, Account, Narrative, Amount) 
									VALUES (21, ' . $CreditNoteNo . ", '" . $SQLCreditNoteDate . "', " . $PeriodNo . ', ' . 
									$EnteredGRN->GLCode . ", '" . $_SESSION['SuppTrans']->SupplierID . ' - ' . 
									_('GRN Credit Note') . ' ' . $EnteredGRN->GRNNo . " - " . $EnteredGRN->ItemDescription . 
									' x ' . $EnteredGRN->This_QuantityInv . ' x ' . _('price var') . ' ' . 
									number_format(($EnteredGRN->ChgPrice  / $_SESSION['SuppTrans']->ExRate),2) . "', " . 
									(-$PurchPriceVar) . ')';
					...
					$SQL = 'INSERT INTO GLTrans (Type, TypeNo, TranDate, PeriodNo, Account, Narrative, Amount) 
							 VALUES (21, ' . $CreditNoteNo . ", '" . $SQLCreditNoteDate . "', " . $PeriodNo . ', ' . 
							 $_SESSION['SuppTrans']->GRNAct . ", '" . $_SESSION['SuppTrans']->SupplierID . ' - ' . 
							 _('GRN') .' ' . $EnteredGRN->GRNNo . ' - ' . $EnteredGRN->ItemCode . ' x ' . 
							 $EnteredGRN->This_QuantityInv . ' @ ' . $_SESSION['SuppTrans']->CurrCode . 
							 $EnteredGRN->ChgPrice . ' @ ' . _('a rate of') . ' ' . $_SESSION['SuppTrans']->ExRate . "', " . 
							 round(-$EnteredGRN->ChgPrice * $EnteredGRN->This_QuantityInv,2) / $_SESSION['SuppTrans']->ExRate . ')';
					
					$ErrMsg = _('ERROR! NOTE DOWN THIS ERROR AND SEEK ASSISTANCE: 
									The general ledger transaction could not be added because');
			...
			if ($_SESSION['SuppTrans']->OvGST != 0){
				...
				$SQL = 'INSERT INTO GLTrans (Type, TypeNo, TranDate, PeriodNo, Account, Narrative, Amount) 
						 VALUES (21, ' . $CreditNoteNo . ", '" . $SQLCreditNoteDate . "', " . $PeriodNo . ', ' . 
						 $_SESSION['SuppTrans']->TaxGLCode . ", '" . $_SESSION['SuppTrans']->SupplierID . ' - ' . 
						 _('Credit Note') . ' ' . $_SESSION['SuppTrans']->SuppReference . ' ' . 
						 $_SESSION['SuppTrans']->CurrCode . $_SESSION['SuppTrans']->OvGST  . ' @ ' . _('a rate of') . ' ' . 
						 $_SESSION['SuppTrans']->ExRate . "', " . round(-$_SESSION['SuppTrans']->OvGST/ $_SESSION['SuppTrans']->ExRate,2) . ')';

				$ErrMsg = _('ERROR! NOTE DOWN THIS ERROR AND SEEK ASSISTANCE: 
								The general ledger transaction for the tax on the suppliers credit note could not be added because');
			...
			$SQL = 'INSERT INTO GLTrans (Type, TypeNo, TranDate, PeriodNo, Account, Narrative, Amount) 
					 VALUES (21, ' . $CreditNoteNo . ", '" . $SQLCreditNoteDate . "', " . $PeriodNo . ', ' . 
					 $_SESSION['SuppTrans']->CreditorsAct . ", '" . $_SESSION['SuppTrans']->SupplierID . ' - ' . 
					 _('Credit Note') . ' ' . $_SESSION['SuppTrans']->SuppReference . ' ' . 
					 $_SESSION['SuppTrans']->CurrCode . number_format($_SESSION['SuppTrans']->OvAmount + $_SESSION['SuppTrans']->OvGST,2)  . ' @ ' . _('a rate of') . ' ' . $_SESSION['SuppTrans']->ExRate . 
					 "', " . round($LocalTotal + ($_SESSION['SuppTrans']->OvGST / $_SESSION['SuppTrans']->ExRate),2) . ')';
					 
			$ErrMsg = _('ERROR! NOTE DOWN THIS ERROR AND SEEK ASSISTANCE: 
							The general ledger transaction for the control total could not be added because');
	...
	/*Now insert the credit note into the SuppTrans table*/
		...
		$SQL = 'INSERT INTO SuppTrans (TransNo, Type, SupplierNo, SuppReference, TranDate, DueDate, OvAmount, OvGST, Rate, TransText)
			...
			'" . $SQLCreditNoteDate . "', '" . FormatDateForSQL($_SESSION["SuppTrans"]->DueDate) . "', " . 
			round(-$_SESSION['SuppTrans']->OvAmount,2) . ', ' .(-$_SESSION['SuppTrans']->OvGST) . ', ' .  
			$_SESSION['SuppTrans']->ExRate . ", '" . $_SESSION['SuppTrans']->Comments . "')";

		$ErrMsg = _('CRITICAL ERROR! NOTE DOWN THIS ERROR AND SEEK ASSISTANCE: 
						The supplier credit note transaction could not be added to the database because');
		...
		foreach ($_SESSION['SuppTrans']->GRNs as $EnteredGRN){
			...
			$ErrMsg = _('ERROR! NOTE DOWN THIS ERROR AND SEEK ASSISTANCE: 
							The quantity credited off the goods received record could not be updated because');
			...
			if (strlen($EnteredGRN->ShiptRef)>0 AND $EnteredGRN->ShiptRef!=0){
				...
				$SQL = 'INSERT INTO ShipmentCharges (ShiptRef, TransType, TransNo, StockID, Value) VALUES (' . 
						 $EnteredGRN->ShiptRef . ', 21, ' . $CreditNoteNo . ", '" . $EnteredGRN->ItemCode . "', " . 
						 round(-$EnteredGRN->This_QuantityInv * $EnteredGRN->ChgPrice / $_SESSION['SuppTrans']->ExRate,2) . ')';
						 
				$ErrMsg = _('ERROR! NOTE DOWN THIS ERROR AND SEEK ASSISTANCE: 
								The shipment charge record for the shipment') . ' ' . $EnteredGRN->ShiptRef . ' ' . _('could not be added because');
		...
		foreach ($_SESSION['SuppTrans']->Shipts as $ShiptChg){
			...
			$SQL = 'INSERT INTO ShipmentCharges (ShiptRef, TransType, TransNo, Value) VALUES (' . 
					 $ShiptChg->ShiptRef . ', 21, ' . $CreditNoteNo . ', ' . (-$ShiptChg->Amount/$_SESSION['SuppTrans']->ExRate) . ')';

			$ErrMsg = _('ERROR! NOTE DOWN THIS ERROR AND SEEK ASSISTANCE: 
							The shipment charge record for the shipment') . ' ' . $ShiptChg->ShiptRef . ' ' . _('could not be added because');
		...
		echo '<P>' . _('Supplier credit note number') . ' ' . $CreditNoteNo . ' ' . _('has been processed') . '<BR>';
