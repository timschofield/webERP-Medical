...
$Title = _('Notes Types') . ' / ' . _('Maintenance');
...
echo '<p class="page_title_text">
		<img src="', $RootPath, '/css/', $_SESSION['Theme'], '/images/maintenance.png" title="', _('Notes Types'), '" alt="" />', _('Notes Type Setup'), '
	</p>';
echo '<div class="page_help_text">', _('Add/edit/delete Notes Types'), '</div>';
...
if (isset($_POST['Insert']) or isset($_POST['Update'])) {
	...
	if (isset($_POST['Update']) and $InputError != 1) {
		...
		if ($CheckRow[0] > 0 and !isset($SelectedType)) {
			...
			$SQL = "INSERT INTO care_type_notes (nr,
													type,
													name,
													status,
													create_id,
	...
	$SQL = "DELETE FROM care_type_notes WHERE nr='" . $SelectedType . "'";
	...
	if (DB_num_rows($Result) > 0) {
		...
		while ($MyRow = DB_fetch_array($Result)) {
			echo '<tr class="striped_row">
					...
					<td><a href="', htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'), '?SelectedType=', urlencode($MyRow['nr']), '&amp;delete=yes" onclick="return MakeConfirm(\'' . _('Are you sure you wish to delete this Notes Type?') . '\', \'Confirm Delete\', this);">' . _('Delete') . '</a></td>
				</tr>';
		}
		//END WHILE LIST LOOP
		echo '</tbody>';
...
if (!isset($_GET['delete'])) {
	...
	if (isset($SelectedType) and $SelectedType != '') {
		...
		echo '<fieldset>
				<legend>', _('Edit Notes Type'), ' - ', $MyRow['type'], '</legend>';
		...
		echo '<fieldset>
				<legend>', _('Create New Notes Type'), '</legend>';
	}
	...
	if (isset($SelectedType) and $SelectedType != '') {
		echo '<div class="centre">
				<input type="submit" name="Update" value="', _('Update Notes Type'), '" />
			</div>';
	} else {
		echo '<div class="centre">
				<input type="submit" name="Insert" value="', _('Create Notes Type'), '" />
			</div>';
	}
